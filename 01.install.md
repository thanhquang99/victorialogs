...
sudo curl -L -O https://github.com/VictoriaMetrics/VictoriaMetrics/releases/latest/download/vmauth-linux-amd64
sudo chmod +x vmauth

sudo useradd --system --no-create-home --shell /usr/sbin/nologin vmauth
sudo chown -R vmauth:vmauth /opt/vmauth /etc/vmauth
```